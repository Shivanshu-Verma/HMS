...
export default function CounsellorPatientsPage() {
  ...
  useEffect(() => {
          ...
          const mapped = data.items.map(
            (item: any) =>
              ({
                ...
                addiction_duration:
                  item.addiction_duration_text || item.addiction_duration,
              }) as Patient,
          );
          setPatients(mapped);
        })
    ...
    loadPatients();
    const refreshTimer = window.setInterval(loadPatients, 15000);
    const onFocus = () => loadPatients();
  ...
  const handleSaveEdit = async () => {
    ...
    if (
      accessToken &&
      selectedPatient &&
      editingPatient.status !== selectedPatient.status
    ) {
      try {
        await updatePatientStatus(
          accessToken,
          editingPatient.id,
          editingPatient.status as "active" | "inactive" | "dead",
        );
      } catch (error) {
        toast.error(
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">
            View complete patient profiles and session history
          </p>
        </div>
            ...
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-xl font-bold">
                  {
                    patients.filter((p) => p.patient_category === "psychiatric")
                      .length
                  }
                </p>
          ...
          <Card>
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-xl font-bold">
                  {
                    patients.filter((p) => p.patient_category === "deaddiction")
                      .length
                  }
                </p>
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-3 mt-0.5 text-xs text-muted-foreground">
                              ...
                              {patient.hdams_id && (
                                <span>HDAMS: {patient.hdams_id}</span>
                              )}
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-0 shadow-lg overflow-hidden">
        <div
          className={`h-1.5 ${isPsychiatric ? "bg-gradient-to-r from-purple-500 to-indigo-500" : "bg-gradient-to-r from-[#0d7377] to-[#14919b]"}`}
        />
        <CardContent className="p-6">
          <div className="flex items-start gap-5">
            <div
              className={`h-16 w-16 rounded-2xl flex items-center justify-center shrink-0 text-white font-bold text-xl ${isPsychiatric ? "bg-gradient-to-br from-purple-500 to-indigo-600" : "bg-gradient-to-br from-[#0d7377] to-[#14919b]"}`}
            >
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                >
                  ...
                  {selectedPatient.patient_category
                    ? PATIENT_CATEGORY_LABELS[selectedPatient.patient_category]
                    : "N/A"}
                </Badge>
                <Badge
                  variant={
      ...
      <Tabs defaultValue="profile">
        <TabsList className="grid w-full grid-cols-3">
          ...
          <TabsTrigger value="sessions">
            Session History ({patientSessions.length})
          </TabsTrigger>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Card>
              ...
              <CardContent className="pt-0 space-y-0">
                ...
                <InfoRow
                  label="Aadhaar No."
                  value={selectedPatient.aadhaar_number}
                />
                <InfoRow
                  label="Nationality"
                ...
                <InfoRow
                  ...
                  value={
                    selectedPatient.marital_status
                      ? MARITAL_STATUS_LABELS[selectedPatient.marital_status]
                      : undefined
                  }
                />
                <InfoRow
            ...
            <Card>
              ...
              <CardContent className="pt-0 space-y-2">
                {selectedPatient.substance_used_currently &&
                  selectedPatient.substance_used_currently.length > 0 && (
                    <div>
                ...
                {selectedPatient.substance_ever_used &&
                  selectedPatient.substance_ever_used.length > 0 && (
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">
                        Ever Used
                      </p>
                ...
                <InfoRow
                  label="Injection (Ever)"
                  value={
                    selectedPatient.injection_use_ever
                      ? "Yes"
                      : selectedPatient.injection_use_ever === false
                        ? "No"
                        : undefined
                  }
                />
                <InfoRow
                  ...
                  value={
                    selectedPatient.injection_use_currently
                      ? "Yes"
                      : selectedPatient.injection_use_currently === false
                        ? "No"
                        : undefined
                  }
                />
                <InfoRow
                  ...
                  value={
                    selectedPatient.syringe_sharing
                      ? "Yes"
                      : selectedPatient.syringe_sharing === false
                        ? "No"
                        : undefined
                  }
                />
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm flex items-center gap-2">
                  <HeartPulse className="h-4 w-4 text-primary" /> Medical
                  History
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-0 space-y-0">
                ...
                <InfoRow
                  label="Jaundice"
                  value={
                    selectedPatient.jaundice
                      ? "Yes"
                      : selectedPatient.jaundice === false
                        ? "No"
                        : undefined
                  }
                />
                <InfoRow
                  ...
                  value={
                    selectedPatient.hiv_screening
                      ? "Yes"
                      : selectedPatient.hiv_screening === false
                        ? "No"
                        : undefined
                  }
                />
                ...
                <InfoRow
                  label="Co-morbid Medical"
                  value={selectedPatient.comorbid_medical_illness}
                />
                <InfoRow
                  label="Co-morbid Psychiatric"
                  value={selectedPatient.comorbid_psychiatric_illness}
                />
                <InfoRow
                  label="Previous Treatment"
                  value={
                    selectedPatient.previous_drug_treatment ||
                    selectedPatient.previous_treatments
                  }
                />
                <InfoRow
                  label="Ever Hospitalized"
                  value={
                    selectedPatient.ever_hospitalized
                      ? "Yes"
                      : selectedPatient.ever_hospitalized === false
                        ? "No"
                        : undefined
                  }
                />
          ...
          {patientSessions.length > 0 ? (
            ...
            <Card>
              <CardContent className="py-12 text-center text-muted-foreground">
                ...
                <p className="text-sm">
                  Session history will appear here after counselling
                </p>
              </CardContent>
            </Card>
          )}
          ...
          {patientVisits.length > 0 ? (
            ...
            <Card>
              <CardContent className="py-12 text-center text-muted-foreground">
                ...
                <p className="text-sm">
                  Visit history will appear here after check-in
                </p>
              </CardContent>
            </Card>
          )}
            ...
            <div className="space-y-6 py-2">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v) => handleEditChange("blood_group", v)}
                    >
                      <SelectTrigger>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleEditChange("religion", e.target.value)
                      }
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v) => handleEditChange("education", v)}
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Select education" />
                      </SelectTrigger>
                      <SelectContent>
                        {Object.entries(EDUCATION_LABELS).map(([k, l]) => (
                  ...
                  <div>
                    ...
                    >
                      ...
                      <SelectContent>
                        {Object.entries(MARITAL_STATUS_LABELS).map(([k, l]) => (
                          <SelectItem key={k} value={k}>
                            {l}
                          </SelectItem>
                        ))}
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-3 mt-3">
                  ...
                  <label className="flex items-center gap-2 text-sm cursor-pointer">
                    <Checkbox
                      checked={editingPatient.injection_use_currently || false}
                      onCheckedChange={(c) =>
                ...
                <div className="mt-3">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleEditChange("route_of_admission", e.target.value)
                    }
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <Input
                      value={editingPatient.comorbid_psychiatric_illness || ""}
                      onChange={(e) =>
                        handleEditChange(
                  ...
                  <div>
                    <Label>Previous Treatment for Drug Abuse</Label>
                    <Input
                  ...
                  <div className="col-span-2 flex flex-wrap gap-4">
                    <label className="flex items-center gap-2 text-sm cursor-pointer">
                      <Checkbox
                        ...
                        onCheckedChange={(c) => handleEditChange("jaundice", c)}
                      />
                      Jaundice
                    </label>
                    <label className="flex items-center gap-2 text-sm cursor-pointer">
